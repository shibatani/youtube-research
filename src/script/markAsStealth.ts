...
// ============================
// 設定: ステルス認定したいYouTubeチャンネルID (UC...) を指定
// ============================
...
const main = async () => {
  ...
  await channel.bulkMarkAsStealth(targetChannels.map(({ id }) => id));

  const nameList = targetChannels.map(({ name }) => `• ${name}`).join("\n");
  console.log(`ステルス認定したチャンネル一覧:\n${nameList}`);

  const message = `[markAsStealth] ステルス認定完了\n• 対象: ${targetChannels.length}件\n${nameList}`;
  console.log(message);
